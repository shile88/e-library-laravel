...
<div class="height-autori pb-[30px] scroll" style="overflow-y:auto; max-height:70vh;">
    ...
    <div class="inline-block min-w-full px-[30px] pt-3 align-middle bg-white rounded-bl-lg rounded-br-lg">
                ...
                <tr class="border-b-[1px] border-[#e4dfdf]">
                    ...
                    @foreach ($headers as $header)
                        @if ($header === 'Book name' || $header === 'Full name' || $header === 'Author name')
                            <th class="px-4 py-4 leading-4 tracking-wider text-left">{{ $header }}<a
                                    href="#"><i class="ml-3 fa-lg fas fa-long-arrow-alt-down"
                                        onclick="sortTable()"></i></a>
                            </th>
                        @elseif($header === 'Author')
                            <th class="px-4 py-4 leading-4 tracking-wider text-left">{{ $header }}<a
                                    href="#"><i class="ml-3 fas fa-filter" onclick="sortTable()"></i></a>
                            </th>
                        @elseif($header === 'Category')
                            <th class="px-4 py-4 leading-4 tracking-wider text-left">{{ $header }}<a
                                    href="#"><i class="ml-3 fas fas fa-filter" onclick="sortTable()"></i></a>
                            </th>
                        @else
                ...
                @foreach ($items as $item)
                    <tr class="hover:bg-gray-200 hover:shadow-md border-b-[1px] border-[#e4dfdf]">
                        ...
                        <td class="flex flex-row items-center px-4 py-3">
                            <img class="object-cover w-8 mr-2 h-11" src="{{ asset('/storage/' . $item->picture) }}"
                                alt="" />
                        ...
                        @isset($pivots)
                            @foreach ($pivots as $pivot)
                                <td class="px-4 py-3 text-sm leading-5 whitespace-nowrap">
                        ...
                        <td class="px-4 py-3 text-sm leading-5 text-right whitespace-nowrap">
                            <p
