...
@section('content')
    ...
    <form class="text-gray-700" method="post" action="{{ route('bindings.update', $binding) }}">
        @csrf
        @method('put')
        <div class="flex flex-row ml-[30px]">
            <div class="w-[50%]">
                {{-- We need this field to go into GenreStoreRequest for duplicate validation --}}
                <input hidden type="number" name="id" value="{{ $binding->id }}" />

                {{-- Name --}}
                <div class="mt-[20px]">
                    <p>Name<span class="text-red-500">*</span></p>
                    <input type="text" name="name" required value="{{ $binding->name }}"
                        ...
                        placeholder="Enter a binding name" />
    ...
    {{-- Delete button --}}
    <div class="flex flex-row ml-[30px]">
        <form action="{{ route('bindings.destroy', $binding) }}" method="post">
            @csrf
            @method('delete')
